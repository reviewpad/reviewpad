// Copyright 2022 Explore.dev Unipessoal Lda. All Rights Reserved.
// Use of this source code is governed by a license that can be
// found in the LICENSE file.

package plugins_aladino_functions

import (
	"regexp"
	"strings"

	"github.com/reviewpad/reviewpad/v3/lang/aladino"
)

func Changed() *aladino.BuiltInFunction {
	return &aladino.BuiltInFunction{
		Type: aladino.BuildFunctionType([]aladino.Type{aladino.BuildStringType(), aladino.BuildStringType()}, aladino.BuildBoolType()),
		Code: changedCode,
	}
}

func changedCode(e aladino.Env, args []aladino.Value) (aladino.Value, error) {
	antecedentRegex := args[0].(*aladino.StringValue).Val
	consequentRegex := args[1].(*aladino.StringValue).Val

	patA, patB := diffStrings(antecedentRegex, consequentRegex)

	antecedentMatches := getMatches(e, antecedentRegex)
	consequentMatches := getMatches(e, consequentRegex)

	retValue := aladino.BuildTrueValue()

	for varName, antecedentVals := range antecedentMatches {

		consequentVals, ok := consequentMatches[varName]
		if !ok {
			return aladino.BuildFalseValue(), nil
		}

		// check that every element in storedVal is in newVal
		for _, leftVal := range antecedentVals {
			if patA == "" && strings.HasSuffix(leftVal, patB) {
				continue
			}

			leftValExists := false
			for _, rightVal := range consequentVals {
				if leftVal == rightVal {
					leftValExists = true
					break
				}
			}
			if !leftValExists {
				return aladino.BuildFalseValue(), nil
			}
		}
	}

	return retValue, nil
}

func getMatches(e aladino.Env, pattern string) map[string][]string {
	resolvedPattern, vars := interpolateRegex(pattern)
	re := regexp.MustCompile(resolvedPattern)

	valsMatrix := make(map[string][]string, 0)

	for fp := range e.GetPatch() {
		for idx, ranges := range re.FindAllStringSubmatchIndex(fp, -1) {
			lower := ranges[2]
			upper := ranges[3]
			varName := vars[idx]
			val := fp[lower:upper]
			vals, ok := valsMatrix[varName]
			if !ok {
				valsMatrix[varName] = []string{val}
			} else {
				valsMatrix[varName] = append(vals, val)
			}
		}
	}

	return valsMatrix
}

func diffStrings(a, b string) (string, string) {
	if len(a) >= len(b) {
		return ".", ""
	}

	indexLeft := 0
	indexRight := len(b)
	gap := len(b) - len(a)

	for i := 0; i < len(a); i++ {
		if a[i] != b[i] {
			indexLeft = i
			break
		}
	}

	for j := len(b) - 1; j >= 0; j-- {
		if a[j-gap] != b[j] {
			indexRight = j + 1
			break
		}
	}

	return a[indexLeft : indexRight-gap], b[indexLeft:indexRight]
}

func interpolateRegex(s string) (string, []string) {
	vars := make([]string, 0)
	var sb strings.Builder

	re := regexp.MustCompile(`@[0-9]*\.?[0-9]+`)
	index := 0

	matches := re.FindAllStringIndex(s, -1)
	for _, ranges := range matches {
		lower := ranges[0]
		upper := ranges[1]

		sb.WriteString(s[index:lower])
		index = upper

		varName := s[lower:upper]

		sb.WriteString("(.*)")
		vars = append(vars, varName)

	}

	if index < len(s) {
		sb.WriteString(s[index:])
	}

	return sb.String(), vars
}
